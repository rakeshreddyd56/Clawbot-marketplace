#!/usr/bin/env bash
# Auto-generated runner script for backend-2 agent
...
agents.append({'name': 'backend-2', 'role': 'backend-2', 'status': 'working', 'current_task': '', 'session_start': '${NOW}', 'last_heartbeat': '${NOW}'})
reg['agents'] = agents
...
exec claude \
  ...
  -p "You are the backend-2 agent. Read .claude/coordination/TASKS.md and begin working on your assigned tasks. Update task status as you progress."
