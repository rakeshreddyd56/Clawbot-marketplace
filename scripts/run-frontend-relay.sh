#!/usr/bin/env bash
# Auto-relay runner script for frontend agent — task TASK-ENFORCE-004
...
agents = [a for a in reg['agents'] if a.get('name') != 'frontend']
agents.append({'name': 'frontend', 'role': 'frontend', 'status': 'working', 'current_task': 'TASK-ENFORCE-004', 'session_start': '${NOW}', 'last_heartbeat': '${NOW}'})
reg['agents'] = agents
with open(reg_path, 'w') as f: json.dump(reg, f, indent=2)
" 2>/dev/null || true
fi

SYSTEM_PROMPT="You are the frontend agent for the \"Clawbot-marketplace\" project.
...
== COLLABORATION ==
You are on Floor 2 (Dev Floor). Your lead is rataa-frontend (Nami). Report progress via send-message to rataa-frontend. Coordinate with backend-1 and backend-2 on API integration. Ask tester-2 to validate your work.
...
CREATE TASK:
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"create-task\",\"projectId\":\"clawbot-marketplace\",\"title\":\"TITLE\",\"description\":\"DESC\",\"status\":\"TODO\",\"priority\":\"P1\",\"agentId\":\"frontend\"}'

MOVE TASK STATUS (TODO, IN_PROGRESS, REVIEW, DONE, etc.):
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"move-task\",\"projectId\":\"clawbot-marketplace\",\"taskId\":\"TASK_ID\",\"status\":\"DONE\",\"agentId\":\"frontend\"}'

COMMENT ON TASK:
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"comment-task\",\"projectId\":\"clawbot-marketplace\",\"taskId\":\"TASK_ID\",\"agentId\":\"frontend\",\"content\":\"COMMENT\"}'

SEND MESSAGE TO AGENT:
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"send-message\",\"projectId\":\"clawbot-marketplace\",\"fromAgent\":\"frontend\",\"toAgent\":\"AGENT_ID\",\"content\":\"MSG\"}'
...
exec claude \
  ...
  -p "Work on task TASK-ENFORCE-004: Implement dispute response deadline (72h auto-ruling). Read .claude/coordination/TASKS.md for details. Update task status as you progress."
