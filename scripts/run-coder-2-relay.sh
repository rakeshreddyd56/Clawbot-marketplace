#!/usr/bin/env bash
# Auto-relay runner script for coder-2 agent — task TASK-HARD-005
...
agents = [a for a in reg['agents'] if a.get('name') != 'coder-2']
agents.append({'name': 'coder-2', 'role': 'coder-2', 'status': 'working', 'current_task': 'TASK-HARD-005', 'session_start': '${NOW}', 'last_heartbeat': '${NOW}'})
reg['agents'] = agents
with open(reg_path, 'w') as f: json.dump(reg, f, indent=2)
" 2>/dev/null || true
fi

SYSTEM_PROMPT="You are the coder-2 agent for the \"Clawbot-marketplace\" project.
...
== COLLABORATION ==
Work on parallel tasks to coder. Focus on frontend/UI if coder handles backend. Avoid file conflicts by checking locked files.
...
== DASHBOARD API ==
Check board: curl -s 'http://localhost:4000/api/agent-actions?action=board-summary&projectId=clawbot-marketplace'
Move task: curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"move-task\",\"projectId\":\"clawbot-marketplace\",\"taskId\":\"TASK_ID\",\"status\":\"DONE\"}'
Send message: curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"send-message\",\"projectId\":\"clawbot-marketplace\",\"fromAgent\":\"coder-2\",\"toAgent\":\"AGENT\",\"content\":\"MSG\"}'"

exec claude \
  --system-prompt "$SYSTEM_PROMPT" \
  --allowedTools "Read,Write,Edit,Bash,Grep,Glob" \
  -p "Work on task TASK-HARD-005: Enforce signed Stripe webhook verification  idempotency. Read .claude/coordination/TASKS.md for details. Update task status as you progress."
