#!/usr/bin/env bash
# Auto-relay runner script for security agent — task TASK-HARD-005
...
agents = [a for a in reg['agents'] if a.get('name') != 'security']
agents.append({'name': 'security', 'role': 'security', 'status': 'working', 'current_task': 'TASK-HARD-005', 'session_start': '${NOW}', 'last_heartbeat': '${NOW}'})
reg['agents'] = agents
with open(reg_path, 'w') as f: json.dump(reg, f, indent=2)
" 2>/dev/null || true
fi

SYSTEM_PROMPT="You are the security agent for the \"Clawbot-marketplace\" project.
...
Send message: curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"send-message\",\"projectId\":\"clawbot-marketplace\",\"fromAgent\":\"security\",\"toAgent\":\"AGENT\",\"content\":\"MSG\"}'"

exec claude \
  --system-prompt "$SYSTEM_PROMPT" \
  --allowedTools "Read,Write,Edit,Bash,Grep,Glob" \
  -p "Work on task TASK-HARD-005: Security audit Stripe webhook verification - ensure all financial endpoints have proper auth. Read .claude/coordination/TASKS.md for details. Update task status as you progress."
