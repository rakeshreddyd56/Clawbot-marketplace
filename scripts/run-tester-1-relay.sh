#!/usr/bin/env bash
# Auto-relay runner script for tester-1 agent — task TASK-HARD-003
...
agents = [a for a in reg['agents'] if a.get('name') != 'tester-1']
agents.append({'name': 'tester-1', 'role': 'tester-1', 'status': 'working', 'current_task': 'TASK-HARD-003', 'session_start': '${NOW}', 'last_heartbeat': '${NOW}'})
reg['agents'] = agents
with open(reg_path, 'w') as f: json.dump(reg, f, indent=2)
" 2>/dev/null || true
fi

SYSTEM_PROMPT="You are the tester-1 agent for the \"Clawbot-marketplace\" project.
...
== COLLABORATION ==
You are on Floor 2 (Dev Floor). Your lead is rataa-backend (Franky). Write and run tests for backend-1 and backend-2 work. Report failures via send-message to rataa-backend and the failing agent. Create bug tickets for failures.
...
CREATE TASK:
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"create-task\",\"projectId\":\"clawbot-marketplace\",\"title\":\"TITLE\",\"description\":\"DESC\",\"status\":\"TODO\",\"priority\":\"P1\",\"agentId\":\"tester-1\"}'

MOVE TASK STATUS (TODO, IN_PROGRESS, REVIEW, DONE, etc.):
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"move-task\",\"projectId\":\"clawbot-marketplace\",\"taskId\":\"TASK_ID\",\"status\":\"DONE\",\"agentId\":\"tester-1\"}'

COMMENT ON TASK:
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"comment-task\",\"projectId\":\"clawbot-marketplace\",\"taskId\":\"TASK_ID\",\"agentId\":\"tester-1\",\"content\":\"COMMENT\"}'

SEND MESSAGE TO AGENT:
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"send-message\",\"projectId\":\"clawbot-marketplace\",\"fromAgent\":\"tester-1\",\"toAgent\":\"AGENT_ID\",\"content\":\"MSG\"}'
...
exec claude \
  ...
  -p "Work on task TASK-HARD-003: Run tests and typecheck for all completed tasks. Read .claude/coordination/TASKS.md for details. Update task status as you progress."
