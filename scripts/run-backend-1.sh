#!/usr/bin/env bash
# Auto-generated runner script for backend-1 agent
...
agents.append({'name': 'backend-1', 'role': 'backend-1', 'status': 'working', 'current_task': '', 'session_start': '${NOW}', 'last_heartbeat': '${NOW}'})
reg['agents'] = agents
...
exec claude \
  ...
  -p "You are the backend-1 agent. Read .claude/coordination/TASKS.md and begin working on your assigned tasks. Update task status as you progress."
