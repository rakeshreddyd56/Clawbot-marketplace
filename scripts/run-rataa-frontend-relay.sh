#!/usr/bin/env bash
# Auto-relay runner script for rataa-frontend agent — task frontend-qa
...
agents = [a for a in reg['agents'] if a.get('name') != 'rataa-frontend']
agents.append({'name': 'rataa-frontend', 'role': 'rataa-frontend', 'status': 'working', 'current_task': 'frontend-qa', 'session_start': '${NOW}', 'last_heartbeat': '${NOW}'})
reg['agents'] = agents
with open(reg_path, 'w') as f: json.dump(reg, f, indent=2)
" 2>/dev/null || true
fi

SYSTEM_PROMPT="You are the rataa-frontend agent for the \"Clawbot-marketplace\" project.
...
CREATE TASK:
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"create-task\",\"projectId\":\"clawbot-marketplace\",\"title\":\"TITLE\",\"description\":\"DESC\",\"status\":\"TODO\",\"priority\":\"P1\",\"agentId\":\"rataa-frontend\"}'

MOVE TASK STATUS (TODO, IN_PROGRESS, REVIEW, DONE, etc.):
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"move-task\",\"projectId\":\"clawbot-marketplace\",\"taskId\":\"TASK_ID\",\"status\":\"DONE\",\"agentId\":\"rataa-frontend\"}'

COMMENT ON TASK:
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"comment-task\",\"projectId\":\"clawbot-marketplace\",\"taskId\":\"TASK_ID\",\"agentId\":\"rataa-frontend\",\"content\":\"COMMENT\"}'

SEND MESSAGE TO AGENT:
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"send-message\",\"projectId\":\"clawbot-marketplace\",\"fromAgent\":\"rataa-frontend\",\"toAgent\":\"AGENT_ID\",\"content\":\"MSG\"}'
...
exec claude \
  ...
  -p "Work on task frontend-qa: QA review all frontend components and pages for completeness. Read .claude/coordination/TASKS.md for details. Update task status as you progress."
