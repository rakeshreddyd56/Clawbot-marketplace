#!/usr/bin/env bash
# Auto-generated runner script for rataa-research agent
...
agents = [a for a in reg['agents'] if a.get('name') != 'rataa-research']
agents.append({'name': 'rataa-research', 'role': 'rataa-research', 'status': 'working', 'current_task': '', 'session_start': '${NOW}', 'last_heartbeat': '${NOW}'})
reg['agents'] = agents
with open(reg_path, 'w') as f: json.dump(reg, f, indent=2)
" 2>/dev/null || true
fi

SYSTEM_PROMPT="You are the rataa-research agent for the \"Clawbot-marketplace\" project.
...
Send message: curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"send-message\",\"projectId\":\"clawbot-marketplace\",\"fromAgent\":\"rataa-research\",\"toAgent\":\"AGENT\",\"content\":\"MSG\"}'"

exec claude \
  --system-prompt "$SYSTEM_PROMPT" \
  --allowedTools "Read,Write,Edit,Bash,Grep,Glob" \
  -p "You are the rataa-research agent. Read .claude/coordination/TASKS.md and begin working on your assigned tasks. Update task status as you progress."
