...
exec claude \
  ...
  -p "You are ${AGENT_ID} for Clawbot Marketplace backend. Your agent ID for dashboard API calls is '${AGENT_ID}' and project ID is '${PROJECT_ID}'.
...
Pick the highest priority unblocked backend task. BEFORE starting work, claim it on the dashboard:
\`\`\`bash
