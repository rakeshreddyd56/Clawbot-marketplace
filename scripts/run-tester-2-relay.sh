#!/usr/bin/env bash
# Auto-relay runner script for tester-2 agent — task build-check
...
agents = [a for a in reg['agents'] if a.get('name') != 'tester-2']
agents.append({'name': 'tester-2', 'role': 'tester-2', 'status': 'working', 'current_task': 'build-check', 'session_start': '${NOW}', 'last_heartbeat': '${NOW}'})
reg['agents'] = agents
with open(reg_path, 'w') as f: json.dump(reg, f, indent=2)
" 2>/dev/null || true
fi

SYSTEM_PROMPT="You are the tester-2 agent for the \"Clawbot-marketplace\" project.
...
== COLLABORATION ==
You are on Floor 2 (Dev Floor). Your lead is rataa-frontend (Nami). Write E2E and regression tests for frontend work. Report failures via send-message to rataa-frontend and the failing agent. Coordinate with tester-1 on shared test infrastructure.
...
CREATE TASK:
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"create-task\",\"projectId\":\"clawbot-marketplace\",\"title\":\"TITLE\",\"description\":\"DESC\",\"status\":\"TODO\",\"priority\":\"P1\",\"agentId\":\"tester-2\"}'

MOVE TASK STATUS (TODO, IN_PROGRESS, REVIEW, DONE, etc.):
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"move-task\",\"projectId\":\"clawbot-marketplace\",\"taskId\":\"TASK_ID\",\"status\":\"DONE\",\"agentId\":\"tester-2\"}'

COMMENT ON TASK:
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"comment-task\",\"projectId\":\"clawbot-marketplace\",\"taskId\":\"TASK_ID\",\"agentId\":\"tester-2\",\"content\":\"COMMENT\"}'

SEND MESSAGE TO AGENT:
  curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"send-message\",\"projectId\":\"clawbot-marketplace\",\"fromAgent\":\"tester-2\",\"toAgent\":\"AGENT_ID\",\"content\":\"MSG\"}'
...
exec claude \
  ...
  -p "Work on task build-check: Run npm run build and npm run lint to verify all packages compile. Read .claude/coordination/TASKS.md for details. Update task status as you progress."
