#!/usr/bin/env bash
# Auto-generated runner script for researcher-1 agent
...
agents = [a for a in reg['agents'] if a.get('name') != 'researcher-1']
agents.append({'name': 'researcher-1', 'role': 'researcher-1', 'status': 'working', 'current_task': '', 'session_start': '${NOW}', 'last_heartbeat': '${NOW}'})
reg['agents'] = agents
with open(reg_path, 'w') as f: json.dump(reg, f, indent=2)
" 2>/dev/null || true
fi

SYSTEM_PROMPT="You are the researcher-1 agent for the \"Clawbot-marketplace\" project.
...
Send message: curl -s -X POST http://localhost:4000/api/agent-actions -H 'Content-Type: application/json' -d '{\"action\":\"send-message\",\"projectId\":\"clawbot-marketplace\",\"fromAgent\":\"researcher-1\",\"toAgent\":\"AGENT\",\"content\":\"MSG\"}'"

exec claude \
  --system-prompt "$SYSTEM_PROMPT" \
  --allowedTools "Read,Write,Edit,Bash,Grep,Glob" \
  -p "You are the researcher-1 agent. Read .claude/coordination/TASKS.md and begin working on your assigned tasks. Update task status as you progress."
