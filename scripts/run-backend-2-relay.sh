#!/usr/bin/env bash
# Auto-relay runner script for backend-2 agent — task TASK-HARD-004
...
agents.append({'name': 'backend-2', 'role': 'backend-2', 'status': 'working', 'current_task': 'TASK-HARD-004', 'session_start': '${NOW}', 'last_heartbeat': '${NOW}'})
reg['agents'] = agents
