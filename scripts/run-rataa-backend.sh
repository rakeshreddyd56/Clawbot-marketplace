#!/usr/bin/env bash
# Auto-generated runner script for rataa-backend agent
...
agents.append({'name': 'rataa-backend', 'role': 'rataa-backend', 'status': 'working', 'current_task': '', 'session_start': '${NOW}', 'last_heartbeat': '${NOW}'})
reg['agents'] = agents
...
exec claude \
  ...
  -p "You are the rataa-backend agent. Read .claude/coordination/TASKS.md and begin working on your assigned tasks. Update task status as you progress."
